...
print("fusion");
executeQuery(db.games_with_reviews, [ { $lookup: { from: "commercial_data", localField: "name", foreignField: "Title", as: "commercial_info" } }, { $unwind: "$commercial_info" }, { $match: { "overall_player_rating": "Overwhelmingly Positive" } }, { $addFields: { clean_discounted_price: { $cond: { if: { $eq: ["$commercial_info.Discounted Price", "Free"] }, then: 0, else: { $toDouble: { $trim: { input: { $replaceAll: { input: "$commercial_info.Discounted Price", find: ",", replacement: "" } }, chars: " $" } } } } } } }, { $match: { clean_discounted_price: { $lt: 10 } } }, { $sort: { clean_discounted_price: 1 } }, { $limit: 10 }, { $project: { _id: 0, title: "$name", discounted_price: "$commercial_info.Discounted Price" } } ]);
print("overload");
